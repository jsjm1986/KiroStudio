...
/// 测试用的全局串行锁。守护这几把 key 的**唯一**一把锁，不要在别处再建一把。
///
...
///
/// 日后若别处的用例也要动这些 key，必须从这里取锁：各建一个私有 `static SERIAL`
/// 就是**两把不同的锁守同一份状态**——等于没有互斥，测试会随机失败且极难复现。
#[cfg(test)]
...
mod tests {
    ...
    /// 四把 key 相互独立：写一把不得影响另几把（曾用同一 cell 会串味）。
    ///
    /// 【为何专门锁这条】四把 key 共用同一套 `matches`/`set` 代码，若 `cell()` 的
    /// match 分支写错（复制粘贴时漏改一处），会出现「用 importApiKey 也能打开管理面」
    ...
    fn keys_are_independent() {
        ...
        // 反向：每把 key 都不得当成另外任何一把用（穿透是双向的，只查一个方向会漏）。
        assert!(!admin_key_matches("user-1"), "userKey 不得打开管理面");
        assert!(!user_key_matches("relay-1"));
        assert!(!import_key_matches("admin-1"));
    }
